...
class LanguageModelGUI:
    ...
    def generate_text_gui(self, model, input_text, max_length=50, temperature=1.0, top_k=0, top_p=0.0, repetition_penalty=1.0):
        ...
        with torch.autograd.set_detect_anomaly(True):
            ...
            logging.debug(f"Logit prime shape: {logit_prime.shape}")
            logging.info(f"Logit prime before sigmoid: {logit_prime[:10]}") 
            ...
            logging.info(f"Logit probs after sigmoid: {probs[:10]}") 
            ...
            try:
                ...
                logging.info(f"Output logit prime for text projection Values:\n{logit_prime}")
                logging.info(f"Output probs for text projection Values:\n{probs}")
                logging.info(f"Output logit prime for text projection Shape: {logit_prime.shape}")
                logging.info(f"Output probs for text projection Shape: {probs.shape}")

                sampled_binary = sample_from_probabilities(probs, threshold=threshold)
