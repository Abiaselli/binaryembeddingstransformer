...
EOS= "ÿÿÿÿ"
EOS_TOKEN = "11111111 11111111 11111111 11111111" # 40 digits
EOS_BINARY = 0o11111111111111111111111111111111 # 40 digits
...
class MiniTransformerNode(nn.Module):
    ...
    def forward(self, x, prev_node_output=None, src_mask=None, is_final_node=False):
        ...
        x = torch.clamp(x.long(), min=0, max=self.hidden_size)
...
class BinaryCascadeTransformer(nn.Module):
    ...
    def forward(self, input_text, mask=None):
        ...
        binary_embeddings=self.output_layer(binary_embeddings)
        ...
        with torch.autograd.set_detect_anomaly(True):
            for epoch in range(self.epochs.get()):
                ...
                for idx, (query, target) in enumerate(dataset):  # Process individual pairs
                    ...
                    try:
                        binary_embeddings = self.model.preprocessing_node(query)  # Convert to list for preprocessing
                    ...
                    logging.debug(f"binary embeddings shape {binary_embeddings.shape}")
                    logging.debug(f"binary embedding {binary_embeddings}")

                    # Forward pass: Preprocess queries within the model

                    
                    batch_size, seq_len, hidden_size = binary_embeddings.shape
                    try:
                    # Generate attention mask
                        mask = generate_attention_mask(binary_embeddings, self.num_heads.get(), self.max_seq_length.get()).to(self.device)

                    except Exception as e:
                        logging.error(f"Error during mask generation: {e}")
                        raise
                    # Debug logs
                    logging.debug(f"Generated mask shape: {mask.shape}, binary embeddings shape: {binary_embeddings.shape}")
