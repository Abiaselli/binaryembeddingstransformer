...
class UnifiedTransformerGUI:
    def __init__(self, root):
        ...
        self.max_seq_length = tk.IntVar(value=max_seq_len)
    ...
    def create_widgets(self):
        ...
        ttk.Label(transformer_frame, text="Max seq length (don't touch):").grid(row=3, column=4, sticky="w")
    ...
    def training_loop(self):
        ...
        # Create DataLoader with your desired batch size
        dataloader = DataLoader(
            dataset,
            batch_size=self.batch_size.get(),
            shuffle=True,
            collate_fn=custom_collate_fn  # define or import this function
        )
        ...
        with torch.autograd.set_detect_anomaly(True):
            for epoch in range(self.epochs.get()):
                ...
                for batch_idx, (query_list, target_list) in enumerate(dataloader):
                    if self.stop_training.is_set():
                        logging.info("Training stopped by user.")
                        messagebox.showinfo("Info", "Training stopped by user.")
                        return
                    ...
                    # 1) Preprocess each query in the batch
                    #    e.g. queries => [batch_size, seq_len, binary_len]
                    #    same for targets
                    query_tensors = []
                    target_tensors = []
                    for q in query_list:
                        q_tensor = preprocess_text(q, max_seq_len=max_seq_len)  # your new shortened seq_len
                        query_tensors.append(q_tensor)

                    for t in target_list:
                        t_tensor = preprocess_text(t, max_seq_len=max_seq_len)
                        target_tensors.append(t_tensor)
                    logging.debug(f"Query tensor sample: {query_tensors[:2]}")
                    logging.debug(f"Target tensor sample: {target_tensors[:2]}")
                    
                    # 2) Stack them into a single batch dimension:
                    #    shape => [batch_size, seq_len, binary_len]
                    # Convert query_list and target_list to tensors
                    queries_batch = preprocess_batch(query_list, max_seq_len=max_seq_len, chunk_size=32).to(self.device)
                    targets_batch = preprocess_batch(target_list, max_seq_len=max_seq_len, chunk_size=32).to(self.device)

                    logging.debug(f"Query batch shape: {queries_batch.shape}")
                    logging.debug(f"Target batch shape: {targets_batch.shape}")
                    logging.debug(f"Query batch: {queries_batch[:2]}")
                    logging.debug(f"Query batch list: {list(queries_batch[:2])}")
                    logging.debug(f"Target batch: {targets_batch[:2]}")
                    logging.debug(f"Target batch list: {list(targets_batch[:2])}")
                    logging.debug(f"Query batch length: {len(queries_batch)}")
                    logging.debug(f"Target batch length: {len(targets_batch)}")                    
                    logging.debug(f"Query batch list length: {len(list(queries_batch))}")
                    logging.debug(f"Target batch list length: {len(list(targets_batch))}")

                    # 3) Forward pass
                    try:
                    # Forward pass through the model
                        logits, attention_weights = self.model(queries_batch, mask=None)
                        logging.debug(f"Logits shape: {logits.shape}")

                        # Now `logits` should be [batch_size, seq_len, binary_len, 1] or similar
                    except Exception as e:
                    ...
                    # 4) Convert targets into the same shape (for BCE)
                    targets_projected = self.model.target_projection(targets_batch)
                    # targets_projected should be [batch_size, seq_len, binary_len, 1] as well
                    ...
                    logging.debug(f"Targets: {targets_flat[:1]}")
                ...
                logging.info(f"Epoch {epoch+1}/{self.epochs.get()} - Loss: {avg_loss:.4f}")
